...
test('v76 build markers stay aligned', () => {
  ...
  const version = 'v76-20260901-patient-visual-cues';
...
test('patient-simple HUD is restricted to public Levels 5 and 6', () => {
  ...
  assert.match(html,
    /@media \(min-width:601px\) and \(max-width:900px\) and \(orientation:portrait\)[\s\S]*?\.game-stage\.patient-simple-hud \.prompt-zone\{ top:156px !important; \}/);
...
test('open and closed hand cues use large emoji plus text', () => {
  ...
  assert.match(html, /Math\.max\(64, Math\.min\(94,/);
...
test('large canvas cues and advanced enlargements are public-only', () => {
  ...
  assert.match(html, /const tileMin = patientLarge \? \(narrow \? 62 : 70\) : \(narrow \? 56 : 64\)/);
...
test('Level 5 and 6 objects and targets are enlarged without changing gesture gates', () => {
  ...
  assert.match(html,
    /const level6Boost = \(!research\.active && state\.level === '67'\)[\s\S]*?state\.theme === 'peg_laundry' \? 1\.10 : 1\.16/);
  assert.match(html, /patientTargetBoost[\s\S]*?state\.theme === 'peg_laundry' \? 1\.10 : 1\.16/);
  assert.match(html, /\[\[0\.20,0\.40\],\[0\.50,0\.40\],\[0\.80,0\.40\]\]/);
