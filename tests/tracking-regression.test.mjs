...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v73-20260830-bowling/);
});
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v73-20260830-bowling/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v73-20260830-bowling"'));
  assert.equal(manifest.start_url, "./index.html?build=v73-20260830-bowling");
