...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v108-20260907-peg-release-cheer-cartoons/);
});
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v108-20260907-peg-release-cheer-cartoons/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v108-20260907-peg-release-cheer-cartoons"'));
  assert.equal(manifest.start_url, "./index.html?build=v108-20260907-peg-release-cheer-cartoons");
