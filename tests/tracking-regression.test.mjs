...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v104-20260905-landing-copy-horizontal-cert/);
});
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v104-20260905-landing-copy-horizontal-cert/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v104-20260905-landing-copy-horizontal-cert"'));
  assert.equal(manifest.start_url, "./index.html?build=v104-20260905-landing-copy-horizontal-cert");
