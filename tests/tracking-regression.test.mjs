...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v107-20260907-lone-hand-grasp-stability/);
});
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v107-20260907-lone-hand-grasp-stability/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v107-20260907-lone-hand-grasp-stability"'));
  assert.equal(manifest.start_url, "./index.html?build=v107-20260907-lone-hand-grasp-stability");
