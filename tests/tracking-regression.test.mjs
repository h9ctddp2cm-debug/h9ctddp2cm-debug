...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v74-20260831-teahouse/);
});
...
test("Levels 2–4 open the shared activity library with Level 2 fail-closed to bilateral", () => {
  ...
  assert.match(publicSource, /const THEME_ORDER = \['wipewindow','bowling','mahjongwash','buspay','bowlinglane','flowers','dimsum','laundry','fridge','cards','mahjong','cooking'\]/);
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v74-20260831-teahouse/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v74-20260831-teahouse"'));
  assert.equal(manifest.start_url, "./index.html?build=v74-20260831-teahouse");
