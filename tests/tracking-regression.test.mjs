...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v88-20260902-chopstick-small-range-fix/);
});
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v88-20260902-chopstick-small-range-fix/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v88-20260902-chopstick-small-range-fix"'));
  assert.equal(manifest.start_url, "./index.html?build=v88-20260902-chopstick-small-range-fix");
