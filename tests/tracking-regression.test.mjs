...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v103-20260905-l3-demo-palmar-grasp/);
});
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v103-20260905-l3-demo-palmar-grasp/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v103-20260905-l3-demo-palmar-grasp"'));
  assert.equal(manifest.start_url, "./index.html?build=v103-20260905-l3-demo-palmar-grasp");
