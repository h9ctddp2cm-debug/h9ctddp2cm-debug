...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v112-20260912-public-no-recording/);
});
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v112-20260912-public-no-recording/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v112-20260912-public-no-recording"'));
  assert.equal(manifest.start_url, "./index.html?build=v112-20260912-public-no-recording");
