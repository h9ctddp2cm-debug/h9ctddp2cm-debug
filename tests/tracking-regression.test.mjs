...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v99-20260904-hand-check-switch-lock-privacy-blur/);
});
...
test("Levels 3 and 4 can use Pose when tabletop hands occlude the finger model", () => {
  ...
  assert.match(publicSource, /function needsGrossPoseInference\(\)/);
  assert.match(publicSource, /const res = needsGrossPoseInference\(\)\s*\? readGrossPoseHand\(grossPose\) : readHand\(frame\)/);
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v99-20260904-hand-check-switch-lock-privacy-blur/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v99-20260904-hand-check-switch-lock-privacy-blur"'));
  assert.equal(manifest.start_url, "./index.html?build=v99-20260904-hand-check-switch-lock-privacy-blur");
