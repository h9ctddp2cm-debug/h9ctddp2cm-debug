...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v101-20260904-level56-fridge-laundry-cards/);
});
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v101-20260904-level56-fridge-laundry-cards/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v101-20260904-level56-fridge-laundry-cards"'));
  assert.equal(manifest.start_url, "./index.html?build=v101-20260904-level56-fridge-laundry-cards");
