...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v113-20260912-public-no-certificate/);
});
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v113-20260912-public-no-certificate/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v113-20260912-public-no-certificate"'));
  assert.equal(manifest.start_url, "./index.html?build=v113-20260912-public-no-certificate");
