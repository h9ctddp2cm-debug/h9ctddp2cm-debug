...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v81-20260902-patient-usability/);
});
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v81-20260902-patient-usability/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v81-20260902-patient-usability"'));
  assert.equal(manifest.start_url, "./index.html?build=v81-20260902-patient-usability");
