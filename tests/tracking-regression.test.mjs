...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v102-20260905-kungfu-theme-certificate/);
});
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v102-20260905-kungfu-theme-certificate/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v102-20260905-kungfu-theme-certificate"'));
  assert.equal(manifest.start_url, "./index.html?build=v102-20260905-kungfu-theme-certificate");
