...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v85-20260902-remove-personalised-range-label/);
});
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v85-20260902-remove-personalised-range-label/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v85-20260902-remove-personalised-range-label"'));
  assert.equal(manifest.start_url, "./index.html?build=v85-20260902-remove-personalised-range-label");
