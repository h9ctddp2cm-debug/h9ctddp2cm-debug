...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v110-20260907-peg-hold-hysteresis-gate/);
});
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v110-20260907-peg-hold-hysteresis-gate/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v110-20260907-peg-hold-hysteresis-gate"'));
  assert.equal(manifest.start_url, "./index.html?build=v110-20260907-peg-hold-hysteresis-gate");
