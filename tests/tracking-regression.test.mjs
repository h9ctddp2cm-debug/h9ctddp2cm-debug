...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v100-20260904-privacy-person-segmentation/);
});
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v100-20260904-privacy-person-segmentation/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v100-20260904-privacy-person-segmentation"'));
  assert.equal(manifest.start_url, "./index.html?build=v100-20260904-privacy-person-segmentation");
