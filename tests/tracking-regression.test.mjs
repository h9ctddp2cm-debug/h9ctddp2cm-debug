...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v79-20260901-calib-feedback/);
});
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v79-20260901-calib-feedback/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v79-20260901-calib-feedback"'));
  assert.equal(manifest.start_url, "./index.html?build=v79-20260901-calib-feedback");
