...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v109-20260907-cheer-cartoon-equal-height/);
});
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v109-20260907-cheer-cartoon-equal-height/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v109-20260907-cheer-cartoon-equal-height"'));
  assert.equal(manifest.start_url, "./index.html?build=v109-20260907-cheer-cartoon-equal-height");
