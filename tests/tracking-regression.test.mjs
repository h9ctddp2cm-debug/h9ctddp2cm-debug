...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v111-20260907-trial-result-training-complete/);
});
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v111-20260907-trial-result-training-complete/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v111-20260907-trial-result-training-complete"'));
  assert.equal(manifest.start_url, "./index.html?build=v111-20260907-trial-result-training-complete");
