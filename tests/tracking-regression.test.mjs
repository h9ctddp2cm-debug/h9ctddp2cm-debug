...
test("portrait phones keep the camera inline in a compact preview instead of fullscreen", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v105-20260905-therapist-gifs-orange-tags/);
});
...
test("offline worker forces the current build instead of serving the stale game", () => {
  ...
  assert.match(serviceWorkerSource, /fthue-rehab-v105-20260905-therapist-gifs-orange-tags/);
  assert.ok(publicSource.includes('const LEVEL_APP_BUILD = "v105-20260905-therapist-gifs-orange-tags"'));
  assert.equal(manifest.start_url, "./index.html?build=v105-20260905-therapist-gifs-orange-tags");
