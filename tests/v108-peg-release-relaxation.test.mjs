...
test('calibration derivation: public peg 50%/62%, research keeps 62%/74%', () => {
  ...
  // v110: entry ratio is 40% for the public peg, 56% elsewhere
  assert.match(publicSource,
    /const enterRatio = publicPegReleaseRelaxationEnabled\(\) \? PUBLIC_PEG_ENTER_RATIO : 0\.56;/);
  assert.match(publicSource, /enter: closedMean \+ gapC \* enterRatio,/);
});
...
test('dual-digit rule survives; decisive path is gated and needs both digits to move', () => {
  ...
  // v110: every release path is additionally gated by the combined hysteresis
  assert.match(fn, /: \(\(bothReopened \|\| decisiveReopen\) && pegGateOpen\);/);
});
...
function extractPeg({research = {active: false}, personal = null, pegTask = true} = {}){
  ...
  const factory = new Function('hasFiniteHandLandmarks', 'research', 'state', 'nowMs', 'isLevel6RealToolTask',
    defaults[0] + '\n' + adapt[0] + '\n' + fn[0] +
    '\nreturn {computeToolPinchState, publicPegEffectiveThresholds, publicPegReleaseRelaxationEnabled, publicPegNearFloor, publicPegReleaseGate, toolPinchAdapt};');
...
// v110: combined closed 0.26 (0.68·0.25 + 0.32·0.28), range 0.20 → entry at
// 40% = 0.34, release gate = entry + 10% = 0.36 (below the 50% digit points).
const CAL = {
  enter: 0.34,
  ...
  comboClosed: 0.26, comboRange: 0.20,
};
...
const CAL_WIDE = {
  ...
  comboClosed: 0.282, comboRange: 0.20,
};
...
test('decisive-digit release: one digit past re-arm + other digit moved ≥30%', () => {
  ...
  const r = computeToolPinchState(syntheticHand(gap(0.34), gap(0.41)), true);
  // far 0.41 ≥ farOpen 0.404; near 0.34 < nearExit 0.35 but ≥ max(0.31, floor 0.304);
  // combined 0.362 clears the v110 gate (0.36)
...
test('relaxation is off outside the public peg task and in research mode', () => {
  ...
  const r = notPeg.computeToolPinchState(syntheticHand(gap(0.34), gap(0.41)), true);
...
test('entry into the pinch is unchanged by the v108 path', () => {
  const {computeToolPinchState} = extractPeg({personal: CAL});
  // combined aperture below enter (0.34) closes; clearly open does not
