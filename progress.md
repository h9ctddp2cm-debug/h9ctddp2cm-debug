...
## 2026-09-04 v101 Level 5/6 content update: fridge (apple/durian, 2× food, lit-zone mode), single-garment laundry layouts, rooster plate for basic chopsticks, 3× card instructions (therapist request D-2)
- Therapist requests (Cantonese, 2026-09-04): fridge tofu → apple (photo supplied), add durian (photo supplied), all foods 2× both inside and outside the fridge, a harder fridge mode (no animals) that splits the fridge into six zones with one randomly lit zone (~1/6 area) that must receive the food (different shoulder elevation angles); Level 5 laundry shows one garment at a time so the up-to-four colour baskets can be enlarged and spread across the display; Level 6 laundry shows one garment at a time and clears the rack after six pieces while saying「又有新衫要晾喇」without lag; basic chopsticks uses the same nostalgic rooster plate as complex mode; card-game instruction text 3× without covering the cards; handedness selection stays required.
- Fridge: `imgFridgeApple` / `imgFridgeDurian` (`img/fridge_apple.png`, `img/fridge_durian.png`) added, tofu removed (13 food defs). `FRIDGE_ROUND_SIZE = 6` foods per round. New `state.fridgeDifficulty` (`basic` = any shelf position, `zones` = lit zone) with `#fridgeDifficultyCard` in the Level 5 panel; `fridgeZoneGrid` (3×2 landscape / 2×3 portrait), `fridgeZoneBounds(rect)` (clamps the grid top below the HUD button band — portrait `max(0.13·ch, 150)`, landscape `max(0.09·ch, 70)` — so a lit top-row cell is never hidden under 返回／暫停／停止; top-row cells are therefore slightly shorter than the other rows), `fridgeZoneFrac(zone, rect)`, `fridgeLitZone` (random, never the same zone twice in a row), `drawFridgeZones` (pulsing lit cell). Food radius doubled inside the fridge (`fridgePlacedRadius`); the waiting tray food uses `fridgeTrayVisualRadius` = min(2×, (lane − 36 px label − 8 px)/2.04) with `fridgeBagSpot` placing the photo at the lane top so photo + name label both fit — a true 2× in portrait, ~1.2× in landscape because the tray lane (`fridgeLaneFrac` 0.26/0.33) cannot hold a 368 px sprite without covering the fridge. `drawItemLabel` now clamps the label above the canvas bottom (previously the fridge tray label was clipped in landscape). Pickup radius unchanged. `img/fridge_tofu.png` remains in the repo/dist but is no longer referenced (harmless; removal deferred).
- Level 5 laundry (public patient-cue mode only, `isLaundrySingleGarmentMode`): `laundryConfig` returns `items:1`; `laundrySingleGarmentLayout(cw, ch, n)` places 2–4 baskets (width 0.34–0.44 cw portrait / 0.27–0.36 cw landscape, cap 460 px) away from the HUD band and the single garment (centre-bottom, 210–330 px). Landscape 4-basket layout = two upper baskets near the midline + two lower baskets at the sides. Instruction text「拿起衣物，放入同色衫籃。」. Research / therapist-view layouts untouched.
- Level 6 laundry: one garment on screen; `LAUNDRY_RACK_SLOTS` six evenly spaced slots on an on-screen rail (`railW = min(rw, 0.96 cw)`, `railY` clamped below `hudSafeTop`, garment width ≤ 0.13 railW so six never overlap); `LAUNDRY_RACK_CAPACITY = 6`, after the sixth garment a 1.8 s applause pause (`LAUNDRY_RACK_CLEAR_DELAY_MS`) then the rack clears and `speak(LAUNDRY_NEW_BATCH_TEXT)`; `laundryBatchesCompleted` counted; timer cleared on stop/restart. Complex-mode orders capped at six pieces. v100 baseline had the basic-mode rack drawn 1.6× wider than the canvas, so hung garments were off-screen — fixed by the rail clamp.
- Level 6 chopsticks basic mode: target `{ type:'dimsum_plate', label:'大碟', img:imgRoosterPlate }` (was vector bowl); hints「中央大碗」→「中央大碟」.
- Cards: `CARDS_BIG_CHIP_FONT_PX = 78` (3 × 26 px), `drawCardsBigChip` (auto-shrinks to 94 % width, min 30 px), `cardsBigChipY` (portrait max(0.14 ch, 150 px), landscape max(0.10 ch, 70 px)) keeps the chip below the HUD buttons and above the card field (0.27 ch). Level 6 card sorting gets `drawLevel6CardsChip`「♥ 放紅心牌堆　♠ 放黑桃牌堆」and the instruction bubble text fixed to「拿起啤牌，放到相同花色牌堆。」(was「放到右邊收牌區」, which described the wrong game).
- Tests: new `tests/v101-level56-visual-updates.test.mjs` (6 tests: layout geometry non-overlap across five viewports, rack constants, rooster plate, chip size/position, L6 chip). Updated `v71-laundry-order`, `v72-fridge`, `v81-patient-usability`, `v82-layout-difficulty-fix` for the new slots/rail/fridge defs. Suite in four bounded groups: 530 passed, 3 intentional skips, 0 failed (`v72-fridge` gained a HUD-clamp test; the tray-radius test now also asserts the name label fits). `tools/checkjs.sh` OK, `node --check service-worker.js`, `git diff --check` clean. Markers `v101-20260904-level56-fridge-laundry-cards` ×3, seven marker tests updated, HUD `perf v101`.
- Public build: `dist/public` rebuilt (sanitized index 713 993 bytes, 192 files); isolation test passed after build; boot check on a fresh static server `typeof window.__qa === 'undefined'`, no console errors, `#fridgeDifficultyCard` present, no research UI.
- iPad Playwright QA (820×1180 / 1180×820): fridge basic + zones, L5 laundry tiers 1–4, L6 rack with 5 and 6 garments, basic chopsticks, L5/L6 cards — screenshots visually inspected in `ych_rehab_qa_artifacts/v101/`.

## 2026-09-05 v102 Hong Kong theme「港式神功修煉」, Level 2 card archived, therapist-cartoon Level 3 demo, chopstick slot fix, participation certificate (therapist request D-3)
- Therapist requests (Cantonese, 2026-09-04/05): dim-sum items must not overlap in the chopstick game; kung-fu themed titles for the landing page and Levels 3–6; a printable certificate after day 10; then five revisions — (1) remove the black edge left of the beef ball, (2) hide (not delete) the FTHUE Level 2 card, (3) turn the therapist cartoon from the Level 2 photo into the Level 3 animated demo, (4) swap「中風上肢復康訓練」/「港式神功修煉」so the clinical title is above and larger, (5) certificate wording: 「參與嘉許狀」,「完成十天的」, no line after 祝賀, 『FTHUE-HK 港式神功修煉』遊戲, no stage names, blank month/day with 2026 kept, transparent logo.
- Chopsticks (Level 6 dim-sum, basic + complex): `makeFoodAt` now places the three plate foods on fixed `slotRatios` (landscape `[0.20,0.30] [0.50,0.34] [0.80,0.30]`, portrait `[0.22,0.45] [0.50,0.45] [0.78,0.45]`) chosen by active-count offset, with the previous grid as fallback, so foods never overlap on any viewport. `img/beefball_real.png` cleaned (chopsticks and dark rim removed by ellipse fit + TELEA inpaint; faint colour bleed at the rim remains).
- Landing page: title block = h1 仁濟醫院職業治療部 → h2.service-title 中風上肢復康訓練 (clamp 20–25 px) → p.brand-title FTHUE-HK 港式神功修煉 (clamp 16–19 px) → p.motto 欲要成功，必先勤功！ → 請選擇 Level 開始修煉. Level 3–6 cards: `<h3 class="kungfu">` 佛光初現／大鵬展翅／如來神掌／萬佛朝宗 with slogans (`.lv-kungfu-sub`) and the clinical name kept in `.lv-clinical`; in-game/result headers still use the clinical names. English strings added to `localization.js` (own renderings of the kung-fu names).
- Level 2: card marked `level-card--archived hidden data-archived-level="2"` (display:none). All Level 2 code, ids, the controller module and the GIF remain; research mode (`#researchLevel`) unaffected. Tests updated to assert the card exists but is not laid out.
- Level 3 active demo: new `img/advanced/level3_therapist_shoulder_30_60.gif` (660×570, 4 frames 0→30→60→30°, ~2.6 s loop) generated from the therapist cartoon cut out of the Level 2 photo (seated side view, cup in the right hand, elbow straight); the assisted demo keeps the SVG, and `shoulder_active_30_60.svg` is retained on disk.
- Certificate: new `certificate.html` (public only; copied by `scripts/build-dist.sh`), A4 landscape `@page`, vertical writing, ink-wash background `img/cert/inkwash_bg.jpg`, 回紋 double border, 直排「證書」+ 中風上肢復康訓練／參與嘉許狀, body text as approved, signature 仁濟醫院職業治療師　鄧姑娘 ／ 二〇二六年　月　日 (year from JS, `?year=` override, month/day blank for handwriting), transparent department logo `img/cert/yan_chai_logo_full.png` bottom-left, 「列印／儲存 PDF」button. Entry: dashed「頒發參與嘉許狀」link (`#lnkCertificate`, new tab) under the level cards on the landing page. No day-N picker (names/dates are handwritten, so nothing to enter).
- Tests: new `tests/v102-kungfu-theme-certificate.test.mjs` (8 tests: markers, title order/size, L2 archived, kung-fu names + EN, L3 GIF, certificate wording/link/build copy, RGBA logo, chopstick slots). `ui-layout` (L2 hidden, headings, 44 px targets skip archived L2, L3 GIF) and `shoulder-flexion-levels` (GIF reference) updated; seven marker tests updated. Suite in four bounded groups: 538 passed, 3 intentional skips, 0 failed. `tools/checkjs.sh` OK, `node --check service-worker.js`, `git diff --check` clean. Markers `v102-20260905-kungfu-theme-certificate` ×3, HUD `perf v102`.
- Public build: `dist/public` rebuilt (sanitized index 717 309 bytes, 196 files incl. `certificate.html`, `img/cert/*`, the new GIF); isolation test passed after build; fresh static server boot `typeof window.__qa === 'undefined'`, no research UI, certificate link present, GIF loads (naturalWidth 660).
- iPad Playwright QA (820×1180 / 1180×820) on dist: landing title block, four visible cards (Level 2 absent), Level 3 therapist GIF, certificate link; certificate print PDF (A4 landscape, 1 page) visually inspected — `ych_rehab_qa_artifacts/v102/`.
- Limitations / notes: Level 2 is hidden, not removed; certificate Latin text (FTHUE-HK) is rotated sideways in the vertical layout; printing requires「背景圖形」enabled in Safari/Chrome; GIF is three key poses (no in-between frames).

## 2026-09-05 v103 Level 3 therapist demo GIF: white running shoes, palmar gross grasp (therapist request)
- Therapist request: in the Level 3 demo the therapist should wear white running shoes (not nursing shoes, no white socks) and the fingers should wrap the body of the cup rather than pinch the handle, because FTHUE Level 3 patients usually use a gross palmar grasp.
- Implementation: regenerated the three-pose sheet (0°/30°/60°) from the same therapist cutout with a handle-less cup held in a whole-hand palmar grasp, white sneakers, trouser hems to the shoe, no socks; rebuilt `img/advanced/level3_therapist_shoulder_30_60.gif` with the same spec (660×570, frames 0→30→60→30°, 750/450/950/450 ms, 128-colour palette, ~255 KB). No markup or logic changes.
- Markers bumped to `v103-20260905-l3-demo-palmar-grasp` ×3 (so the service-worker cache refreshes the GIF), HUD `perf v103`; eight marker tests updated.
- Validation: four test groups 538 passed / 3 skipped / 0 failed; `tools/checkjs.sh`, `node --check service-worker.js`, `git diff --check` clean; `dist/public` rebuilt (196 files), isolation test 5/5 after build, fresh static server: `window.__qa` undefined, 4 visible cards, GIF naturalWidth 660; iPad 1180×820 and 820×1180 screenshots inspected (`ych_rehab_qa_artifacts/v102/v103-home-*.png`).

## 2026-09-05 v104 Landing-page copy simplification and horizontal certificate (therapist request)
- Therapist request: lead line「請選擇 Level 開始修煉」→「請選擇 Level」; 「肩」→「膊頭」on the Level 3/4 cards; remove「患臂離桌｜個人化高位主動活動」(Level 4),「患手握放練習」(Level 5) and「患手三點捏握功能任務」(Level 6); captions under the Level 3/4 demos →「拿杯 (active shoulder flexion)」and「雙手舉棒 (active-assisted shoulder flexion)」; certificate text horizontal instead of vertical.
- Implementation: landing-card copy edited in `index.html` only (in-game headers, research-mode labels and camera-setup hints are untouched); five new EN strings in `localization.js` (old keys retained). `certificate.html` rewritten to a horizontal layout: centred「證書」/ 中風上肢復康訓練 / 參與嘉許狀 with a short rule, left-aligned body with 2-em paragraph indents, signature block bottom-right, transparent logo bottom-left; wording unchanged from v102; page title updated.
- Tests: `ui-layout`, `v102-kungfu-theme-certificate`, `language-switch`, `shoulder-flexion-levels` assertions updated for the new copy and optional `.lv-clinical`; markers bumped to `v104-20260905-landing-copy-horizontal-cert` ×3, HUD `perf v104`.
- Validation: four groups 538 passed / 3 skipped / 0 failed; `tools/checkjs.sh`, `node --check service-worker.js`, `git diff --check` clean; `dist/public` rebuilt (196 files), isolation 5/5 after build, fresh static server: `window.__qa` undefined, no research button, 4 cards, expected captions; iPad 1180×820 / 820×1180 screenshots and certificate A4 landscape PDF (1 page) inspected (`ych_rehab_qa_artifacts/v102/v104-*`).

## 2026-09-05 v105 Therapist cartoon GIFs for all Level 3/4 demos and orange Level tags (therapist request)
- Therapist request: Level 3 cup GIF = frames 1+2 of the approved sheet (0°→30°); Level 4 cup GIF = frames 1→2→3→lift higher (0°→30°→60°→90°); add a matching「舉棒」(active-assisted, both hands on the yellow weighted bar) GIF for Level 3 and Level 4 using the same concept; every「FTHUE Level」tag in the brand orange (same as the title) and bold.
- Assets: two 4-panel sheets generated from the approved v103 character (`l3l4_cup_sheet4.png` side view, `l3l4_bar_sheet4.png` side view rotated ~30° so both hands and the full yellow bar with black end caps are visible, elbows straight; a front-view draft was rejected by the therapist) → four GIFs in `img/advanced/`: `level3_therapist_cup.gif`, `level3_therapist_bar.gif` (2 frames, 900/1000 ms), `level4_therapist_cup.gif`, `level4_therapist_bar.gif` (0°→30°→60°→90°→60°→30°, 800/450/450/950/450/450 ms), all 660×570, 128 colours, 115–383 KB. v103 `level3_therapist_shoulder_30_60.gif` removed; the four `shoulder_*.svg` files are retained but no longer referenced.
- UI: `.level-card .lv-tag` now plain text — orange `var(--orange)` words, transparent background, weight 900, 15 px (therapist rejected the orange-pill/white-text draft); alt texts updated to palmar grasp / bar wording.
- Tests: `ui-layout`, `v102-kungfu-theme-certificate` (new GIF set + orange-tag test), `shoulder-flexion-levels` updated; markers bumped to `v105-20260905-therapist-gifs-orange-tags` ×3, HUD `perf v105`.
- Validation: four groups 539 passed / 3 skipped / 0 failed; `tools/checkjs.sh`, `node --check service-worker.js`, `git diff --check` clean; `dist/public` rebuilt (199 files), isolation 5/5 after build; fresh static server: `window.__qa` undefined, no research button, all four GIFs load (660 px), all five tags colour rgb(224,123,57), transparent background, weight 900; iPad 1180×820 / 820×1180 screenshots inspected (`ych_rehab_qa_artifacts/v102/v105-*`).

## 2026-09-05 v106 Flower names/artwork and 荃灣保齡球場 sign above the pins (therapist request)
- Therapist request: flower names 紅玫瑰／橙玫瑰／粉玫瑰／黃菊花／橙菊花／雛菊（删「小」）／百合／蘭花; new 雛菊／百合／蘭花 pictures with no dropped petals; the「荃灣保齡球場 · 懷舊風情」logo placed above the bowling pins during the bowling game as an orientation-to-place cue for elderly patients.
- Flowers: `FLOWER_KINDS` names `粉紅玫瑰`→`粉玫瑰`, `小雛菊`→`雛菊`; `img/flower_06/07/08.png` regenerated from the therapist photos (border flood-fill white removal, 200 px max edge, 256-colour PNG, 9–10 KB each).
- Bowling: new `img/bowling_sign.png` (845×428 transparent, 62 KB) drawn by `drawBowlingSign()` after the background and before the pins. `bowlingLaneGeometry` now returns `ballTopY`, `pinTopY` and a sign rect: band from the DOM HUD (landscape 84 CSS px / portrait 156 CSS px, scaled to canvas px) down to 8 px above the back-row pin tops, centred on the lane, capped so it never overlaps the 現在／目標 panel (x ≥ 262 CSS px); text fallback「荃灣保齡球場」if the PNG has not loaded. To open that band the rack sits at `pinBaseY=ch*0.540` (was 0.390) with pins `topH*0.40` (was 0.45; still ~54 % taller than the v80 0.26), and the ball rest point (`top` of the progress lane, public only, and the roll `startY`) follows the rack via `ballTopY=pinBaseY-ch*0.05`. Angle judgement, strike state machine and research mode unchanged.
- Tests: new `tests/v106-flowers-bowling-sign.test.mjs` (names, artwork sizes, sign asset/draw order/fallback, ball rest point, v73 hand-off contract); `v73-bowling` geometry test updated (pinBaseY 0.540, sign band/centre/width assertions, loader stubs `imgBowlingSign`, `gameCanvas`); `v81`/`v82` pin-size regexes updated; markers bumped to `v106-20260905-flowers-bowling-sign` ×3, HUD `perf v106`.
- Validation: four groups 544 passed / 3 skipped / 0 failed; `tools/checkjs.sh`, `node --check service-worker.js`, `git diff --check` clean; `dist/public` rebuilt (200 files, sign and flowers in `offline-assets.js`), isolation 5/5 after build; fresh static server: `window.__qa` undefined, no page errors, sign served 200; iPad 1180×820 / 820×1180 Playwright screenshots of bowling start/roll/strike and the flowers palette inspected (`ych_rehab_qa_artifacts/v102/v106-*`).

## 2026-09-07 v107 Lone-hand admission and grasp stability (therapist request)
- Therapist report (HP ProBook 440 G8, Windows 11, Edge, 720p webcam, participant inside 1 m, 30 fps / 10 ms tracking, GPU healthy): in public Level 5 the cursor disappeared for seconds mid-game and the ✋/✊ cue flickered continuously — "完全 lag" rather than slow frames. The `?perf=1` overlay showed the ONLY visible hand being rejected by the v99 size gate (大小拒絕 26) and far gate (遠處拒絕 22) with a label score of 0.58 hovering at the 0.55 floor; each rejection ran out the 750 ms grace and cleared the cursor, and the 60 ms (2-frame) grasp confirmation let the open/closed cue flip on single misclassified frames. Therapist asked for "唔好咁嚴但係又玩到".
- Admission (public Level 5/6 only, `handLockEnabled()` → `!research.active`): when exactly one hand is visible (`loneHand`), the locked-size window widens to `HAND_LOCK_SIZE_LONE_MIN_RATIO 0.40`–`HAND_LOCK_SIZE_LONE_MAX_RATIO 2.50` and the locked size is forgotten after `HAND_LOCK_SIZE_FORGET_LONE_MS 1000` (a hand resting on the lap and coming back looks bigger); a lone hand with a confident affected label (`admitPath==='strict'`) skips the 400 ms far-from-lock debounce. Frames with ≥2 hands keep every v99 value (0.55–1.90, 3 s, 0.30 / 400 ms) so a helper's hand still cannot be substituted. Fix: `farHandSince` is now reset when the debounced hand is admitted, so a later far appearance is debounced afresh instead of being admitted instantly.
- Grace: `PUBLIC_LEVEL56_DETECTION_LOST_GRACE_MS 1500` via `detectionLostGraceMs()` (public Level 5/67 → 1500 ms, everything else `DETECTION_LOST_GRACE_MS 750`); the tracking loop reads the function. Interactions stay paused during the grace exactly as before (stale open state cannot drop or pick up).
- Grasp cue: `PUBLIC_LEVEL5_GRASP_CONFIRM_MS 180` in `stabiliseDetectedGesture` for public Level 5 grasp (~6 frames at 30 fps; was the literal `GESTURE_CONFIRM_MS 60`). Level 6 tool gestures already used 180 ms; research keeps 60 ms.
- Tests: new `tests/v107-lone-hand-grasp-stability.test.mjs` (constants, research guards, 204 ms flip / flicker immunity through `__qa.interpretHand`, 1.5 s grace behaviour, Level 4 keeps 750); `v99` lock tests rewritten for lone vs crowded size gate and lone-strict / ambiguous / crowded re-acquisition; `v98` stabiliser regex; `level67-interactions` wrong-hand case shows the wrong hands for 1700 ms; QA probe `__qa.detectionLostGraceMs()`; markers bumped to `v107-20260907-lone-hand-grasp-stability` ×3, HUD `perf v107`.
- Validation: four groups 549 passed / 3 skipped / 0 failed (59 files); `tools/checkjs.sh`, `node --check service-worker.js`, `git diff --check` clean; `dist/public` rebuilt (200 files), isolation 5/5 after build; fresh static server: `window.__qa` undefined, marker present, no page errors; Playwright iPad 820×1180 / 1180×820 and laptop 1413×653 Level 5 fridge/dimsum screenshots at 1 s after a detector miss show the cursor still held with `perf v107` (`ych_rehab_qa_artifacts/v107-*`).

## 2026-09-07 v108 Level 6 衣夾放手放寬與成績單卡通 (therapist request)
- Therapist report (stroke participant, affected hand, calibration all three steps ticked, public Level 6 real-peg task): items could be PICKED UP but never PUT DOWN — "拎得起，但放手都放唔低". Root cause: release required BOTH digit ratios to clear `nearExit`/`farExit` ≈ 62% of the calibrated closed→open range (v70 literal, adaptive lowering only after enough in-play samples); a hemiparetic hand that opens partially (≈45–55% of the calibrated range on the weaker digit) never reached both points, while entry (`enter`) was met easily.
- Release (public real-peg only, gated by `publicPegReleaseRelaxationEnabled()` = `!research.active && isLevel6RealToolTask()`; calibrated ranges required, chopstick flex path untouched): `PUBLIC_PEG_RELEASE_RATIO 0.50` / `PUBLIC_PEG_REARM_RATIO 0.62` for the calibrated exit/open points; new decisive-digit path `decisiveReopen` — one digit reopens past its re-arm point while the other has still moved ≥ `PUBLIC_PEG_DECISIVE_OTHER_RATIO 0.30` of its own range (both digits must move). Pair-baseline guard `publicPegNearFloor()` = min(nearClosed + 0.62·R, farClosed + `PUBLIC_PEG_PAIR_MARGIN_RATIO 0.12`·R) floors every relaxed NEAR threshold so a single static digit + one drifting landmark can never release an item (never looser than v107 against single-finger drift). Adaptive lowering (`publicPegEffectiveThresholds`) only moves thresholds DOWN towards the observed in-play range, never below `PUBLIC_PEG_ADAPT_FLOOR_RATIO 0.25` of the calibrated range and never touching `enter`. Research mode keeps the 0.62/0.74 literals verbatim; Level 5 untouched.
- HUD: `?perf=1` overlay gains a 衣夾 line (`framePerfToolSnapshot()`, from `lastToolPinchLive` set in `computeToolPinchState`, dropped when >1500 ms old): `衣夾 近指 0.366 放手≥0.325 張開≥0.339  遠指 0.404 放手≥0.364 張開≥0.377  門檻 校準+自適應降低`. The overlay is now hidden explicitly in `endGame` (it was only repainted inside the game loop, so it used to linger over the result screen). HUD string `perf v108`.
- Result screen: two cheering therapist cartoons (`img/advanced/result_cheer_like.png` 250×720 thumbs-up, `img/advanced/result_cheer_go.png` 278×720 fist-pump, generated from the therapist's two reference photos, cropped to equal figure scale, explicit width/height attributes) flank the `.result-card` inside a new `.result-stage#resultStage` row; width `clamp(96px,15vw,190px)`, wrap above the card under 700 px; hidden via `.result-cheer[hidden]` / `.result-stage.research-mode .result-cheer` — `endGame` toggles `research-mode` on the stage so the research/pilot flow is visually unchanged. Alt text present; images are `loading="lazy"`.
- Result screen scroll fix (pre-existing since v107 verified against a v107 copy): with the movement-review panel shown the ~940 px card was centred inside `#screen-result` and its top sat 55 px above the viewport on iPad 1180×820 (unreachable, `overflow-y:auto` cannot scroll negative). `#screen-result{align-items:center;justify-content:flex-start;padding:24px}` + `.result-stage{margin:auto;flex:0 0 auto}` → card top 24 px in landscape/laptop, 171 px in portrait, still centred when short.
- Lesson (sanitizer): `scripts/sanitize-public.cjs` `sanitizeCss` drops any CSS rule whose header (including the comment above it) matches /research|pilot/i. The first `.result-stage{}` comment said "research-mode" and the whole rule vanished from `dist/public`, breaking the row layout only in the public build. Comment reworded ("strict-flow class"), rules split, and `tests/v108-peg-release-relaxation.test.mjs` now asserts the split rules exist and the `.result-stage{` header carries no research/pilot words. Never put those words in CSS comments/selectors meant for the public build.
- QA hooks (source only, stripped from dist): `__qa.perfOverlayRefresh()` (resets the 500 ms throttle, repaints, returns overlay text) and `pinchLive` / `perfTool` / `relaxOn` fields in `__qa.level6ToolState()`. In QA every RAF pass is stale (no camera), so the lost branch clears `lastToolPinchLive` between evaluates; screenshots must be taken within the throttle window right after the synchronous frame burst.
- Tests: new `tests/v108-peg-release-relaxation.test.mjs` (13: constants, gate, pair floor, adaptive floor/direction, decisive-digit release, research literals intact, result-stage markup/CSS, sanitizer-safe comment); markers `v108-20260907-peg-release-cheer-cartoons` ×3 (index `LEVEL_APP_BUILD`, SW `CACHE_VERSION`, manifest start_url) with the 8 marker tests and the `/perf v108 {2}/` regex updated.
- Validation: four groups 562 passed / 3 skipped / 0 failed (60 files: 113 + 168/171 + 172 + 109); `tools/checkjs.sh` 11 blocks OK, `node --check service-worker.js`, `git diff --check` clean; `dist/public` rebuilt (202 files), isolation 5/5 after build; dist: markers ×3, `.result-stage{` present, `__qa` absent, `window.__qa` undefined on a fresh static server; Playwright iPad 820×1180 / 1180×820 and laptop 1413×653: public result screen shows both cartoons with no horizontal overflow and card top ≥ 24 px, research result screen hides them, dist row layout renders the 250×720 / 278×720 assets, peg HUD screenshot shows the 衣夾 line (closed grasp near .270 → open release true) — `ych_rehab_qa_artifacts/v108/v108-qa-contact-sheet.png`.

## 2026-09-07 v109 成績單卡通等高與試玩模式排版 (therapist request)
- Therapist feedback on v108: 「個藍色女仔要同個男仔差唔多高度」, and the department uses 試玩 mode, so the result card has no video panel and is short (~393 px).
- Assets: both cartoons re-composed on one 297×771 transparent canvas. The fist-pump figure is scaled ×1.069 so head-top→feet (716 px) matches the thumbs-up figure; feet sit on the same baseline (y 768), head tops at y 49/51. Equal canvas ⇒ equal rendered scale under any width clamp.
- Markup/CSS: each `<img>` now sits in a `.result-cheer` slot `<div>` (`resultCheerLikeSlot` / `resultCheerGoSlot`) with `align-self:stretch; position:relative`; the image is `position:absolute; width:100%; height:100%; object-fit:contain; object-position:center bottom`, so the figures are never taller than the card (trial mode: figures 393 px = card height on iPad landscape / laptop) while still bottom-aligned next to a tall card (video panel: 459 px on iPad landscape, width-limited). `<700px` wrap keeps static images above the card. Hidden rules for `[hidden]` / `.research-mode` unchanged (they target the slots).
- Markers `v109-20260907-cheer-cartoon-equal-height` ×3 (index `LEVEL_APP_BUILD`, SW `CACHE_VERSION`, manifest start_url) + 8 marker tests, `/perf v109 {2}/`, HUD `perf v109`; `tests/v108-peg-release-relaxation.test.mjs` updated for the slot markup, 297×771 attributes, `align-self:stretch` and `object-fit` rules (13/13).
- Validation: four groups 562 passed / 3 skipped / 0 failed (60 files: 113 + 168/171 + 172 + 109); `tools/checkjs.sh` 11 blocks OK, `node --check service-worker.js`, `git diff --check` clean; `dist/public` rebuilt (202 files), isolation 5/5 after build; dist markers ×3, `.result-cheer img{` rule present, `__qa` absent, `window.__qa` undefined on a fresh static server; Playwright iPad 820×1180 / 1180×820 / laptop 1413×653 × {trial, video, research} on source and trial on dist: both figures identical rendered size and bottom (e.g. landscape trial 151×393 each, bottom 607 = card bottom), research hides both, no horizontal overflow, no page errors — `ych_rehab_qa_artifacts/v108b/v108b-qa-contact-sheet.png`.

## 2026-09-07 v110 晾衫衣夾握持穩定（拎起 40% ＋ 放手閘） (therapist bug report)
- Therapist feedback on v108/v109 at bedside: 「晾衫遊戲我淨係攞住隻夾就已經係偵測鬆開同揸實衣夾」— simply holding the real peg flipped held/released every confirmation window; calibration ticks were all green. 「我記得好耐之前係偵測到㗎」 — correct: v107 worked.
- Root cause (owned as a v108 regression): v108 lowered the per-digit release points to 50 % of each digit's range (decisive path ≈ 40 % combined, adaptive floor down to 25 %), while the combined ENTRY stayed at `closedMean + gapC*0.56`. With `isPinching = prev ? !releaseConfirmed : closeConfirmed`, a static aperture between the release points and the entry point satisfies both `ratio <= enter` and `bothReopened`/`decisiveReopen`, so the state inverts every 180 ms. In v107 release (0.62) sat above entry (0.56), so the overlap did not exist.
- Fix (public peg task only, `publicPegReleaseRelaxationEnabled()`): `PUBLIC_PEG_ENTER_RATIO = 0.40` — calibration now stores `enter = closedMean + gapC*enterRatio` (0.56 kept for research/other tasks) plus `comboClosed`/`comboRange`; `PUBLIC_PEG_HYSTERESIS_RATIO = 0.10` — `publicPegReleaseGate(t) = enter + comboRange*0.10` (legacy object without `comboRange`: `enter + 1e-6`, still strictly above entry); `releaseConfirmed = (bothReopened || decisiveReopen) && pegGateOpen` where `pegGateOpen = ratio >= gate`. Net behaviour: squeeze to ≤ 40 % picks up; anything between 40 % and 50 % is a dead band (a held peg stays held, an open hand never picks up); reopening the combined aperture to ≥ 50 % (≈ both digits at their v108 50 % points) releases. Chopstick flex path, research mode, Level 5 untouched.
- HUD (`?perf=1`): extra line `衣夾 合併 <ratio> 拎起≤<enter> 放手閘≥<gate>` via `framePerfToolSnapshot()` (`lastToolPinchLive` now records `enter`/`gate`).
- Tests: new `tests/v110-peg-hold-hysteresis.test.mjs` (6): source wiring; static-hold sweep 0.20–0.70 combined (equal digits, spreads 0/0.05/0.12/0.30, and with the adaptive tracker forcing the lowest release points) from prev=true and prev=false — zero flips at every aperture; regression case reproducing the v108 calibration object (near 0.35 / far 0.38 vs entry 0.372) now stays held; pick-up / light-hold / dead-band / 50 % release / decisive release still behave; gate inert for non-peg tasks and research. `tests/v108-peg-release-relaxation.test.mjs` CAL objects re-derived for the 40 % entry (`enter 0.34, comboClosed 0.26, comboRange 0.20`; wide `0.362/0.282/0.20`), decisive case moved to near 0.34 so the combined aperture (0.362) clears the gate, source assertions updated to `enterRatio` and `&& pegGateOpen` (13/13).
- Markers `v110-20260907-peg-hold-hysteresis-gate` ×3 (index `LEVEL_APP_BUILD`, SW `CACHE_VERSION`, manifest start_url) + 8 marker tests, `/perf v110 {2}/`, HUD `perf v110`.
- Validation: four groups 568 passed / 3 skipped / 0 failed (61 files: 113 + 168/171 + 171 + 116); `tools/checkjs.sh` 11 blocks OK, `node --check service-worker.js`, `git diff --check` clean; `dist/public` rebuilt (202 files), isolation 5/5 after build; dist markers ×3, gate logic present in the minified bundle, `__qa` absent; Playwright iPad 820×1180 / 1180×820 / laptop 1413×653 on source with `?perf=1`: synthetic v110 calibration, squeeze → held (1 transition), 90 frames (3 s) held at the exact v108 flicker aperture (near 0.35 / far 0.38, combined 0.3596) → 0 transitions even with adaptive lowering active (HUD shows digit release 0.312/0.342 below entry 0.340, gate 0.360), 90 frames light hold → 0 transitions, reopen to 0.40/0.44 → released (1 transition), 60 frames open → 0 transitions, no page errors — `ych_rehab_qa_artifacts/v110/v110-qa-contact-sheet.png`.

## 2026-09-07 v111 試玩完結版面改為「訓練完成」 (therapist request)
- Therapist: 「試玩嗰度可唔可以都變成訓練完成？…唔使講不錄影嗰啲字眼…費事好似此地無銀三百両」. The pilot runs every session in 試玩 mode (no recording, privacy), so for the patient it IS the training session; mentioning "not recording" only draws attention. Recording words should appear only when recording is genuinely running (訓練 mode in future).
- Change (result screen only, `endGame()`): `resultTitle` is `訓練完成` / `Training complete` in both session modes (was `試玩完成` / `Trial complete` in trial); `resultContext` in trial mode now uses the same `Level tag · theme title` line as training (was `· 試玩 · 不錄影／不提示` / `· Trial · no recording or therapist alerts`). Research-mode context line (`participantId · Session n · …`) untouched. Mode buttons, their aria-labels and the training-mode `無聲錄影中 · 不錄頭部` indicator (shown only when `startMovementRecording()` actually starts, i.e. never in trial) unchanged.
- Tests: new `tests/v111-trial-result-training-complete.test.mjs` (3): title assignment, absence of `試玩完成` / `Trial complete` / `不錄影／不提示` / `no recording or therapist alerts`, shared context line, research context and mode-button testids intact.
- Markers `v111-20260907-trial-result-training-complete` ×3 + 8 marker tests, `/perf v111 {2}/`, HUD `perf v111`.
- Validation: four groups 571 passed / 3 skipped / 0 failed (62 files: 113 + 168/171 + 164 + 126); `tools/checkjs.sh` 11 OK, `node --check service-worker.js`, `git diff --check` clean; `dist/public` rebuilt (202 files), isolation 5/5 after build; dist markers ×3, old trial wording absent from the bundle, `__qa` absent; Playwright on source (laptop 1413×653 first, then iPad 1180×820 / 820×1180): real `試玩` button for Level 6 → QA `startGame(peg, 3 s)` → virtual clock → `endGame()`: title `訓練完成`, context `FTHUE Level 6 · 衣夾晾衫`, recording indicator not shown, movement review hidden, no forbidden wording, no page errors; dist: `試玩` button lands on the game library as before — `ych_rehab_qa_artifacts/v111/v111-qa-contact-sheet.png`.

## 2026-09-12 v112 公開版移除錄影功能；「試玩」改名「訓練」 (therapist / IRB request)
- Therapist: 「maybe i better delete the recording function on the website? i am afraid the IRB members will not trust me… change the title of 試玩模式 into 訓練模式 and delete the original 訓練模式 (because the original 訓練模式 has recording… not ethical to record HA patients)」. Clarified: the single remaining button behaves 「同而家嘅所有玩法一樣樣，不過冇錄影」; recording code 「只從公開版移除」.
- Mode buttons (5 `.trial-actions` groups, L2/L3/L4/L5/L6): the `data-session-mode="trial"` button is now labelled `訓練` (aria `FTHUE Level N 訓練`); the recording `data-session-mode="training"` button is relabelled `訓練＋錄影` in source and wrapped in `PUBLIC_BUILD_REMOVE_START: recording training mode` markers, so the public build has exactly one button per level (`.trial-actions button:only-child{ grid-column:1 / -1 }` makes it span the card). Internal identifiers (`trial`, `button-mode-trial-N`, `isTrialMode()`) unchanged to avoid churn.
- Recording module (`movementRecording`, `MediaRecorder`, head-excluded canvas stream, `#recordingIndicator`, `#movementReview` panel + download/delete controls, indicator CSS) wrapped in `PUBLIC_BUILD_REMOVE` markers. New `movementRecordingApi` facade (`available/clear/start/stop`) is the only thing call sites touch; in the public build it is inert and `beginSessionMode()` / `exitToLevelSelection()` force `sessionMode='trial'`, so no code path can record. Research block (`recording_status` params) untouched in source.
- Sanitizer (`scripts/sanitize-public.cjs`): CSS drop for `recording-indicator|movement-review` headers; fail-closed guards on public index for `MediaRecorder|captureStream|createHeadExcluded|recordingIndicator|movementReview|MovementVideo|movementRecording.|downloadMovementVideo`, `data-session-mode="training"|button-mode-training`, and wording `錄影|無聲錄|不錄頭部|試玩|Trial mode|Silent recording`; `sanitizePublicLocalization` also drops recording/試玩/Trial dictionary lines and fails if any remain.
- Wording: `fthue-adaptive-progression.js` practice strings drop 試玩 (`連續成功`/`連續失敗`/`練習進度`/`只顯示練習次數，不提供級別建議。`; EN `Consecutive successes` etc.); `localization.js` removes dead `試玩 · 不錄影／不提示` / `試玩完成`, adds `訓練＋錄影 → Training + recording` (source-only label; scrubbed from public localization).
- Tests: new `tests/v112-public-no-recording.test.mjs` (5; last one reads `dist/public`, skips if not built); `tests/ui-layout.test.mjs`, `tests/v78-public-build-isolation.test.mjs` (public flow now clicks the `trial` button), `tests/adaptive-progression.test.mjs` updated. `__qa.state()` now exposes `sessionMode` (source only).
- Markers `v112-20260912-public-no-recording` ×3 + 8 marker tests, `/perf v112 {2}/`, HUD `perf v112`.
- Validation: four groups 576 passed / 3 skipped / 0 failed (63 files: 113 + 168/171 + 157 + 138); `tools/checkjs.sh` 11 OK, `node --check service-worker.js`, `git diff --check`, `./scripts/build-dist.sh` 202 files, isolation 5/5, v112 public test 5/5, `__qa` absent from dist. Details: security-reviews/v112-security-review.md. QA sheet `ych_rehab_qa_artifacts/v112/v112-qa-contact-sheet.png`: public build laptop 1413×653 / iPad 1180×820 / 820×1180 — one 訓練 button per level, no `#recordingIndicator`/`#movementReview`, body text has no 試玩/錄影; source build keeps both buttons, trial flow (peg, 3 s) ends on `訓練完成` with `sessionMode:'trial'` and indicator never shown; zero page errors.

## 2026-09-12 v113 公開版移除嘉許狀功能 (therapist / IRB request)
- Therapist: 「我諗要喺公開版取消埋嘉許狀，因為嘉許狀係一種對參加者嘅『回饋／獎勵』，但 Protocol、PIS/ICF、Portal 完全冇提。IRB 對『給參加者的任何物品』好敏感（怕構成誘因）」。Same pattern as v112 recording removal: strip from the **public build only**; keep the feature in the source/research tree for a possible future ethics-approved version.
- `index.html`: the `.cert-entry` entry link (`頒發參與嘉許狀`) is now wrapped in `PUBLIC_BUILD_REMOVE_START/END: participation certificate entry` markers, immediately above the existing `研究模式` marker block.
- `scripts/sanitize-public.cjs`: CSS drop extended to `cert-entry` selectors; new fail-closed HTML guard for `cert-entry|lnkCertificate|certificate\.html|嘉許狀`; `sanitizePublicLocalization` also drops/fails on `嘉許狀` and `Certificate of participation` dictionary lines.
- `scripts/build-dist.sh`: no longer copies `certificate.html` into `dist/public`; added a hard-exclusion guard that fails the build if `dist/public/certificate.html` exists; extended the authoring/research grep guard with the certificate identifiers.
- `certificate.html` itself is untouched and still lives in the repo root for the source/research build; it is simply never copied to `dist/public` anymore.
- Tests: new `tests/v113-public-no-certificate.test.mjs` (6: source markers present, source file kept, build script no longer copies it + fails closed, sanitizer guards present, localization keeps the source label, dist has zero certificate traces — skips gracefully if `dist/public` absent). `tests/v102-kungfu-theme-certificate.test.mjs` updated (build-script assertion flipped; new marker-presence assertion).
- Markers `v113-20260912-public-no-certificate` ×3 + 8 marker tests, `/perf v113 {2}/`, HUD `perf v113`.
- Validation: four groups 582 passed / 3 skipped / 0 failed (65 files: 113 + 168/171 + 155 + 146; one v107 grasp-stability test flaked once on timing under parallel load, reran clean standalone ×3 and the full group rerun was clean — unrelated to this change); `tools/checkjs.sh` 11 OK, `node --check service-worker.js`, `git diff --check`, `./scripts/build-dist.sh` → 201 files (one fewer than v112's 202, `certificate.html` no longer shipped), `tests/v78-public-build-isolation.test.mjs` 5/5 + `tests/v113-public-no-certificate.test.mjs` 6/6 after build. QA sheet `ych_rehab_qa_artifacts/v113/v113-qa-contact-sheet.png`: public build (laptop 1413×653 bottom, iPad 1180×820 / 820×1180 full page) ends after the Level 5/6 cards with zero certificate entry anywhere; source build keeps `頒發參與嘉許狀` + `研究模式` at the same position; zero page errors on all 6 combinations.
