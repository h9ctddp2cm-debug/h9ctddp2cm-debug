...
- 點心配對、衣物分類及插花的可搬運物件由身體中線附近開始，放置目標改到患側外下方。由於相機及手部游標以鏡像顯示，左患側對應畫面右外下方，右患側對應畫面左外下方。
...
- QA：1180×820 及 820×1180 均已核對鏡像方向；左患側的籃／花瓶位於畫面右外側，右患側位於畫面左外側。衣物及花材由中線開始；正常流程未選患側時會阻止繼續，選擇後提示清除。

## 2026-08-11：花瓶、麻將及啤牌患側外展方向

- 插花維持花枝由畫面中間拿取；左患側的花瓶放在畫面右側，右患側的花瓶放在畫面左側。
