...
def level3():
    base = cover(Image.open(OUT / "level3_cartoon_clean.png"))
    frames = []
    for index in range(28):
        phase = (index % 14) / 13
        x = round(300 + 82 * phase)
        opacity = 90 + int(130 * (1 - abs(phase - 0.5) * 2))
        frame = arrow(base, (x - 58, 286), (x, 286), opacity, both_ends=True)
        frames.append(frame)
    save_gif(frames, OUT / "level3_towel_abduction.gif")


def level4():
    base = cover(Image.open(OUT / "level4_cartoon_clean.png"))
    frames = []
    for index in range(28):
        phase = (index % 14) / 13
        x = round(382 - 54 * phase)
        y = round(292 - 66 * phase)
        opacity = 90 + int(130 * (1 - abs(phase - 0.5) * 2))
        frame = arrow(base, (x + 38, y + 47), (x, y), opacity, both_ends=True)
        frames.append(frame)
    save_gif(frames, OUT / "level4_safe_slide.gif")
